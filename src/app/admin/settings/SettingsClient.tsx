...
export default function SettingsClient() {
  ...
  const { translate, language: contextLanguage, setLanguage: setContextLanguage } = useLanguage();
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data }) => {
      ...
      if (data.user) {
        ...
        setLangPreference(data.user.user_metadata?.language || 'English');
  ...
  const handleSaveProfile = async () => {
    ...
    setIsSaving(false);
    if (!error) {
      showSuccess(translate('profileUpdated'));
  ...
  const handleSavePreferences = async () => {
    setIsSaving(true);
    const { error } = await supabase.auth.updateUser({
      data: { language: langPreference }
    });
    setIsSaving(false);
    if (!error) {
      showSuccess(translate('gotIt'));
      // Notify the context
      window.dispatchEvent(new CustomEvent('language-updated', { detail: { language: langPreference } }));
    } else {
      alert(error.message);
    }
  };
    ...
    <div className="flex flex-col lg:flex-row gap-6 max-w-[1200px] mx-auto w-full">
      ...
      <div className="w-full lg:w-64 shrink-0">
        <div className="rounded-2xl bg-white dark:bg-[#111C44] shadow-horizon p-4 sticky top-24">
          <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4 px-2">{translate('settingsMenu')}</h2>
          <nav className="flex flex-col gap-1">
            <button onClick={() => setActiveTab('profile')} className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors ${activeTab === 'profile' ? 'bg-[#F4F7FE] dark:bg-[#0B1437] text-navy dark:text-white' : 'text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800/50'}`}>
              <User className={`w-5 h-5 ${activeTab === 'profile' ? 'text-[#E41E26]' : ''}`} /> {translate('myProfile')}
            </button>
            <button onClick={() => setActiveTab('preferences')} className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors ${activeTab === 'preferences' ? 'bg-[#F4F7FE] dark:bg-[#0B1437] text-navy dark:text-white' : 'text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800/50'}`}>
              <MonitorSmartphone className={`w-5 h-5 ${activeTab === 'preferences' ? 'text-[#E41E26]' : ''}`} /> {translate('preferences')}
            </button>
            <button onClick={() => setActiveTab('security')} className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors ${activeTab === 'security' ? 'bg-[#F4F7FE] dark:bg-[#0B1437] text-navy dark:text-white' : 'text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800/50'}`}>
              <Shield className={`w-5 h-5 ${activeTab === 'security' ? 'text-[#E41E26]' : ''}`} /> {translate('security')}
            </button>
            <button onClick={() => setActiveTab('notifications')} className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors ${activeTab === 'notifications' ? 'bg-[#F4F7FE] dark:bg-[#0B1437] text-navy dark:text-white' : 'text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800/50'}`}>
              <Bell className={`w-5 h-5 ${activeTab === 'notifications' ? 'text-[#E41E26]' : ''}`} /> {translate('notifications')}
            </button>
          </nav>
        </div>
      </div>
      ...
      <div className="flex-1">
        ...
        <div className="rounded-2xl bg-white dark:bg-[#111C44] shadow-horizon overflow-hidden">
          ...
          {activeTab === 'profile' && (
            <div className="animate-in fade-in">
              <div className="border-b border-gray-100 dark:border-gray-800 px-6 py-5">
                <h3 className="text-xl font-extrabold text-navy dark:text-white">{translate('profileSettings')}</h3>
                <p className="text-sm text-gray-500 font-medium mt-1">{translate('managePublicProfile')}</p>
              </div>
              <div className="p-6">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-8">
                  ...
                  <div>
                    <h4 className="font-bold text-navy dark:text-white text-lg">{translate('profilePicture')}</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-bold text-navy dark:text-white mb-2">{translate('fullName')}</label>
                  ...
                  <div>
                    <label className="block text-sm font-bold text-navy dark:text-white mb-2">{translate('emailAddress')}</label>
                    <div className="relative">
                    ...
                    <p className="text-[10px] text-gray-400 mt-1">{translate('emailCannotBeChanged')}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-navy dark:text-white mb-2">{translate('phoneNumber')}</label>
                    <div className="relative">
          ...
          {activeTab === 'preferences' && (
            <div className="animate-in fade-in">
              <div className="border-b border-gray-100 dark:border-gray-800 px-6 py-5">
                <h3 className="text-xl font-extrabold text-navy dark:text-white">{translate('preferences')}</h3>
                <p className="text-sm text-gray-500 font-medium mt-1">{translate('customizeAppExperience')}</p>
              </div>
              <div className="p-6 space-y-6">
                <div>
                  <label className="block text-sm font-bold text-navy dark:text-white mb-2">{translate('theme')}</label>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    <button onClick={() => setTheme('light')} className={`border-2 rounded-2xl p-4 flex flex-col items-center gap-2 transition ${theme === 'light' ? 'border-[#E41E26] bg-red-50 dark:bg-red-900/10' : 'border-gray-100 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700'}`}>
                      ...
                      <span className="font-bold text-navy dark:text-white">{translate('light')}</span>
                    </button>
                    <button onClick={() => setTheme('dark')} className={`border-2 rounded-2xl p-4 flex flex-col items-center gap-2 transition ${theme === 'dark' ? 'border-[#E41E26] bg-red-50 dark:bg-red-900/10' : 'border-gray-100 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700'}`}>
                      ...
                      <span className="font-bold text-navy dark:text-white">{translate('dark')}</span>
                    </button>
                    <button onClick={() => setTheme('system')} className={`border-2 rounded-2xl p-4 flex flex-col items-center gap-2 transition ${theme === 'system' ? 'border-[#E41E26] bg-red-50 dark:bg-red-900/10' : 'border-gray-100 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700'}`}>
                      ...
                      <span className="font-bold text-navy dark:text-white">{translate('system')}</span>
                    </button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-navy dark:text-white mb-2">{translate('timezone')}</label>
          ...
          {activeTab === 'security' && (
            <div className="animate-in fade-in">
              <div className="border-b border-gray-100 dark:border-gray-800 px-6 py-5">
                <h3 className="text-xl font-extrabold text-navy dark:text-white">{translate('security')}</h3>
                <p className="text-sm text-gray-500 font-medium mt-1">{translate('updateYourPassword')}</p>
              </div>
              <div className="p-6">
                <div className="max-w-md space-y-5">
                  <div>
                    <label className="block text-sm font-bold text-navy dark:text-white mb-2">{translate('currentPassword')}</label>
                    <div className="relative">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-navy dark:text-white mb-2">{translate('newPassword')}</label>
                    <div className="relative">
                      <Key className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                  ...
                  <div>
                    <label className="block text-sm font-bold text-navy dark:text-white mb-2">{translate('confirmNewPassword')}</label>
                    <div className="relative">
                      <Key className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
          ...
          {activeTab === 'notifications' && (
            <div className="animate-in fade-in">
              <div className="border-b border-gray-100 dark:border-gray-800 px-6 py-5">
                <h3 className="text-xl font-extrabold text-navy dark:text-white">{translate('notificationSettings')}</h3>
                <p className="text-sm text-gray-500 font-medium mt-1">{translate('controlNotificationPreferences')}</p>
              </div>
              <div className="p-6 space-y-6">
                ...
                <div className="flex items-center justify-between p-4 rounded-xl border border-gray-100 dark:border-gray-800">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-bold text-navy dark:text-white">{translate('emailAlerts')}</h4>
                ...
                <div className="flex items-center justify-between p-4 rounded-xl border border-gray-100 dark:border-gray-800">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-bold text-navy dark:text-white">{translate('telegramAlerts')}</h4>
                ...
                <div className="flex items-center justify-between p-4 rounded-xl border border-gray-100 dark:border-gray-800">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-bold text-navy dark:text-white">{translate('weeklyReport')}</h4>
